...
parseHtmlString str =
         let els = parse str in

         List.concatMap (\el -> case el of
                  Element "p" attr content -> parseString (textContent content)
                  Element "div" attr content -> [(textContent content, False)]
                  _ -> []
         ) els |> fromList
